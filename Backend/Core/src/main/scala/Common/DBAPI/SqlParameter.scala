...
object SqlParameter:
  given Encoder[SqlParameter] = deriveEncoder[SqlParameter]

  given Decoder[SqlParameter] = new Decoder[SqlParameter]:
    final def apply(c: HCursor): Decoder.Result[SqlParameter] = for
      dataType <- c.downField("dataType").as[String]
      value <- c.downField("value").as[String]
    yield dataType.toLowerCase match
      ...
      case "datetime" =>
        SqlParameter("DateTime", value) // Add case for DateTime
      case "array[int]" =>
        SqlParameter("Array[Int]", value) // Add case for Array[Int]
      case "array[string]" =>
        SqlParameter("Array[String]", value) // Add case for Array[Int]
